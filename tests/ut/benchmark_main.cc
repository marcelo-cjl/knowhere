...
GetIndexParamSuffix(const std::string& index_type, int nn_descent_niter = 20, int vamana_iters = 10,
    ...
    if (index_type == knowhere::IndexEnum::INDEX_HNSWLIB) {
        ...
        std::string suffix = "_D64_iter" + std::to_string(nn_descent_niter);  // graph_degree=64, nn_descent_niter
        if (build_with_int8) {
            suffix += "_int8build";
        }
        return suffix;
    } else if (index_type == knowhere::IndexEnum::INDEX_GPU_VAMANA) {
        std::string suffix = "_D64_iter" + std::to_string(vamana_iters);
        if (build_with_int8) {
            suffix += "_int8build";
        }
        return suffix;
...
GetIndexConfig(const std::string& index_type, int64_t dim, const std::string& metric, int64_t topk,
               ...
               const std::string& refine_type = "fp16", int ef = 100, bool build_with_int8 = false) {
    knowhere::Json json;
    ...
    if (index_type == knowhere::IndexEnum::INDEX_HNSWLIB) {
        ...
        if (adapt_for_cpu) {
            ...
            if (build_with_int8) {
                json[knowhere::indexparam::BUILD_WITH_INT8] = true;
            }
        }
    } else if (index_type == knowhere::IndexEnum::INDEX_GPU_VAMANA) {
        json[knowhere::indexparam::GRAPH_DEGREE] = 64;
...
RunBuildMode(const DatasetConfig& dataset, const std::string& index_type, int64_t topk, bool adapt_for_cpu,
    ...
    std::string param_suffix = GetIndexParamSuffix(index_type, nn_descent_niter, vamana_iters, sq_type, refine_type, build_with_int8);
    ...
    auto conf = GetIndexConfig(index_type, dim, dataset.metric, topk, adapt_for_cpu, nn_descent_niter, vamana_iters,
                               sq_type, refine_k, refine_type, ef, build_with_int8);
...
PrintUsage(const char* prog) {
    std::cout << "Usage: " << prog << " [options]\n"
              ...
              << "  --build_with_int8   GPU_VAMANA: quantize fp32 to int8 for graph build, keep fp32 for CPU search\n"
...
main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--build_with_int8") {
            build_with_int8 = true;
    ...
    if (mode == "truth") {
        ...
        return RunSearchMode(dataset, index_type, topk, search_times, batch_size, adapt_for_cpu, force_rebuild,
                             nn_descent_niter, vamana_iters, sq_type, refine_k, refine_type, ef, build_with_int8);
    }
}
