...
static void
PrintHnswIndexStats(const std::vector<std::shared_ptr<faiss::cppcontrib::knowhere::Index>>& indexes) {
    for (size_t idx = 0; idx < indexes.size(); ++idx) {
        ...
        const faiss::cppcontrib::knowhere::IndexHNSW* hnsw_index = dynamic_cast<const faiss::cppcontrib::knowhere::IndexHNSW*>(index.get());

        // Check if it's wrapped in IndexRefine
        if (hnsw_index == nullptr) {
            const faiss::cppcontrib::knowhere::IndexRefine* refine_index = dynamic_cast<const faiss::cppcontrib::knowhere::IndexRefine*>(index.get());
            if (refine_index != nullptr) {
                hnsw_index = dynamic_cast<const faiss::cppcontrib::knowhere::IndexHNSW*>(refine_index->base_index);
            }
        }
        ...
        const faiss::cppcontrib::knowhere::HNSW& hnsw = hnsw_index->hnsw;
...
static void
ExportHnswGraph(const std::vector<std::shared_ptr<faiss::cppcontrib::knowhere::Index>>& indexes, const std::string& index_prefix) {
    for (size_t idx = 0; idx < indexes.size(); ++idx) {
        const auto& index = indexes[idx];
        if (index == nullptr) {
            continue;
